...
* **Ansible:** Configuration management tool used to install required packages (Apache HTTPD, PHP, Git) and deploy the application code.
...
1. **Checkout SCM:** Pulls the latest infrastructure and configuration code from GitHub.
2. **Terraform Init:** Initializes the Terraform backend and AWS provider.
3. **Terraform Action:** Provisions the AWS EC2 instance dynamically based on the selected region (`us-east-1`) and updated AMI. Extracts the new public IP address.
4. **Run Ansible Playbook:** Connects to the newly created EC2 instance via SSH and runs `instance.yml` to set up the web server and clone the application repository.
5. **Website Validation:** Automatically runs an HTTP check (cURL) against the new server to ensure it returns a `200 OK` status, confirming successful deployment.
6. **Post Actions:** Cleans up temporary inventory files and outputs the final live URL for the application.
